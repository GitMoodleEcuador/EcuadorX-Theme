...
then
	...
	then
		...
		then
			~/.edxthemevenv/bin/ansible-playbook -c local -i ",localhost" main.yml -e "mode=${1//[-]/} theme_branch=$2 -vvv"
		fi
	...
	then
		...
		then
			~/.edxthemevenv/bin/ansible-playbook -c local -i ",localhost" main.yml -e "mode=${1//[-]/}" -e "theme_branch=$2 theme_repo=$3 -vvv"
		fi
	elif test "$#" -eq 4;
	then
		if [ "$1" == "$mode_branch_custom_repo_remote_edx_host" ];
		then
			~/.edxthemevenv/bin/ansible-playbook -i ",$4" main.yml -e "edx_hosts=$4" -e "mode=${1//[-]/}" -e "theme_branch=$2 theme_repo=$3 -vvv"
        elif [ "$1" == "$mode_branch_custom_repo_remote_group_edx_host" ];
        then
        	~/.edxthemevenv/bin/ansible-playbook -i $INI_FILE main.yml --limit $4 -e "edx_hosts=$4" -e "mode=${1//[-]/}" -e "theme_branch=$2 theme_repo=$3 -vvv"
		fi
	fi
